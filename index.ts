...
const CONFIRM_QUESTIONS_SENT_TOOL: Tool = {
  name: "clarifo_confirm_questions_sent_and_trigger_wait_message", // Renamed for clarity
...
class ClarificationServer {
  public askMultipleClarifications(input: unknown): {
    ...
    try {
      ...
      console.error(chalk.blueBright("\n🤖 [Clarifo MCP Server Log] - askMultipleClarifications Invoked"));
      ...
      console.error(chalk.yellowBright("✅ [Clarifo MCP Server Log] - Questions structured. Preparing response for the calling AI."));
...
This precise sequence is mandatory to satisfy system requirements and achieve the correct wait state.`,
      ...
      console.error(chalk.redBright(`❌ [Clarifo MCP Server Log] - Error in askMultipleClarifications: ${errorMessage}`));
  ...
  public confirmQuestionsSentAndTriggerWaitMessage(input: unknown): { content: Array<{ type: string; text: string }>; isError?: boolean } { // Renamed method
    const data = input as { ai_confirmation_message: string };
    console.error(chalk.cyanBright("\n🤖 [Clarifo MCP Server Log] - clarifo_confirm_questions_sent_and_trigger_wait_message Invoked"));
  ...
  public provideClarificationSummary(input: unknown): { content: Array<{ type: string; text: string }>; isError?: boolean } {
    try {
      ...
      console.error(chalk.magenta("\n📋 [Clarifo MCP Server Log] - provideClarificationSummary Invoked"));
      console.error(chalk.magenta("═".repeat(60)));
      ...
      console.error(chalk.magentaBright("\n📝 Final Summary Processed by Tool:"));
      console.error(chalk.magenta(`${data.final_summary}\n`));
      console.error(chalk.greenBright("✅ [Clarifo MCP Server Log] - Summary processed successfully. Returning confirmation to AI.\n"));
      ...
      const errorMessage = error instanceof Error ? error.message : String(error);
      console.error(chalk.redBright(`❌ [Clarifo MCP Server Log] - Error in provideClarificationSummary: ${errorMessage}`));
...
const server = new Server(
  { name: "clarifo", version: "0.1.4" }, // Incremented version
  { capabilities: { tools: {} } }
);
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  ...
  if (request.params.name === "clarifo_confirm_questions_sent_and_trigger_wait_message") { // Updated tool name
    return clarificationServer.confirmQuestionsSentAndTriggerWaitMessage(parsedArgs); // Updated method name
  }
